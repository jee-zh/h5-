...
class App {
  ...
  navInt () {
    ...
    this.navScrollBox.querySelector('ul').innerHTML = navHtml
    // 导航滚动
    this.handelNavBar()
  }
  handelNavBar () {
    ...
    // 设置悬浮滚动
    this.setFixedNavScroll()
  }
  setFixedNavScroll () {
    // 计算宽度
    var fixNav = document.querySelector('.nav-scroll-inner')
    var itemList = fixNav.getElementsByTagName('ul')[0].children;
    var width=0
    for (var  i = 0; i <itemList.length; i++) {
      var spanLength = itemList[i].querySelector('span').innerText.length
      if (i === 0) {
    ...
    fixNav.getElementsByTagName('ul')[0].style.width = width + 'px'
    // 悬浮导航滚动实例化
    this.navFixedScroll = new BScroll(fixNav, {
      scrollX:true,
      click: true
    })
    this.handleChannelChoice(fixNav)
    // 统一设置两个导航的样式和选中状态，以及滚动到可视区
    this.setFixedNavInt()
  ...
  setFixedNavInt () {
    let lis = document.querySelectorAll('.nav-scroll-inner li')
    let lis2 = document.querySelectorAll('.nav li')
    let screenWidth = -document.body.scrollWidth / 2
    // 默认全部取出选中样式
    for (let k = 0; k < lis.length; k++) {
      (function(k){
        lis[k].classList.remove('active')
        lis2[k].classList.remove('active')
      })(k)
    }
    // 当前选中的加载选中样式
    lis[this.lastIndex].classList.add('active')
    lis2[this.lastIndex].classList.add('active')
    // 滚动到具体可视区
    this.navFixedScroll.scrollToElement(lis[this.lastIndex], 200, screenWidth+80, 0, 'easing')
    this.navScroll.scrollToElement(lis2[this.lastIndex], 200, screenWidth, 0, 'easing')
  }
  ...
  otherBtnClick () {
    // 悬浮logo触发点击跳转
    this.navScrollBox.querySelector('.logo').addEventListener('click', () => {
  ...
  setPageScroll (type) {
    // 设置页面滚动
    var topHeight = this.nav.clientHeight + this.headerTop.clientHeight
    let that = this
    if (!this.bodyScroll) {
      this.bodyScroll = new BScroll(this.pageWrapper, {
        probeType: 3,
